...
/*
	special constants for node->parent 一些常量
*/
	...
	void Graph<captype,tcaptype,flowtype>::maxflow_init()
{
	...
	queue_first[0] = queue_last[0] = NULL;  //初始化队列
	...
	void Graph<captype,tcaptype,flowtype>::test_consistency(node* current_node)
{
	...
	for (i=nodes; i<node_last; i++)
	{
		// test whether all edges in seach trees are non-saturated（不饱和）
		if (i->parent == NULL) {}
